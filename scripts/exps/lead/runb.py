...
class GenMolOpt():
    def __init__(self, args):
        ...
        # --- bandit state ---
        self.Q = defaultdict(float)   # frag -> estimated value
        self.N = defaultdict(int)     # frag -> times selected
        for prop, frag in self.population:
            self.Q[frag] = max(self.Q[frag], prop)   # seed Q with known docking scores
        self.alpha = getattr(self.args, 'q_alpha', 0.4)      # EMA learning rate
        self.epsilon = getattr(self.args, 'epsilon', 0.15)   # exploration rate
    ...
    def select_frag(self, candidates, exclude=None):
        """Epsilon-greedy selection over unique fragments."""
        if exclude is not None:
            candidates = [f for f in candidates if f != exclude]
        if random.random() < self.epsilon or not candidates:
    ...
    def update_bandit(self, frag_pairs, prop_list, reward_idx=0):
        """reward_idx picks which of (rv, rq, rs, rsim) drives Q. Default 0 = rv
        (docking score), matching what update_population already gates on."""
        rewards = prop_list[reward_idx]
        for (frag1, frag2), r in zip(frag_pairs, rewards):
